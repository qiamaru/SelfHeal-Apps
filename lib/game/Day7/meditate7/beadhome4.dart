...
class PrayerBeads4 extends StatefulWidget {
  @override
  _PrayerBeads4State createState() => _PrayerBeads4State();
}

class _PrayerBeads4State extends State<PrayerBeads4> {
  final String kbcc7 = 'beadsCountc7';
  final String kbrc7 = 'RoundCountc7';
  // final String kImagePath = 'imagePath';
  late SharedPreferences prefs4;
  ...
  void initState() {
    SharedPreferences.getInstance().then((SharedPreferences sp) {
      prefs4 = sp; // set an initial value
    });
    Timer(
  ...
  void _loadData() {
    if (!_isDisposed) {
      setState(() {
        _beadCounter = prefs4.getInt(kbcc7) ?? 0;
        _RoundCounter = prefs4.getInt(kbrc7) ?? 0;
        // _imagePath = prefs4.getString(kImagePath) ?? '';
        _accumulatedCounter =
  ...
  void _resetEveryThing() {
    prefs4.setInt(kbcc7, 0);
    prefs4.setInt(kbrc7, 0);
    // prefs4.setString(kImagePath, '');
    _loadData();
  }
  ...
  void _clicked(VoidCallback callback) {
                        ...
                        onPressed: (() {                                                      String uid = FirebaseAuth.instance.currentUser!.uid;
                          ...
                          if (Provider.of<Status>(context, listen: false)
                              ...
                              Provider.of<IsNotGreen>(context, listen: false)
                                  .isnotgreen) {
                            context.read<Status>().chg('d7', 3,uid);
    ...
    prefs4.setInt(kbcc7, _beadCounter);
    prefs4.setInt(kbrc7, _RoundCounter);
