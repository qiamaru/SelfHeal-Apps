...
class PrayerBeads2 extends StatefulWidget {
  @override
  _PrayerBeads2State createState() => _PrayerBeads2State();
}

class _PrayerBeads2State extends State<PrayerBeads2> {
  final String kbcc3 = 'beadsCountc3';
  final String kbrc3 = 'RoundCountc3';
  // final String kImagePath = 'imagePath';
  late SharedPreferences raras;
  ...
  void initState() {
    SharedPreferences.getInstance().then((SharedPreferences sp2) {
      raras = sp2; // set an initial value
    });
    Timer(
 ...
 Expanded(
  ...
  void _loadData() {
    if (!_isDisposed) {
      setState(() {
        _beadCounter = raras.getInt(kbcc3) ?? 0;
        _RoundCounter = raras.getInt(kbrc3) ?? 0;
        // _imagePath = raras.getString(kImagePath) ?? '';
        _accumulatedCounter =
  ...
  void _resetEveryThing() {
    raras.setInt(kbcc3, 0);
    raras.setInt(kbrc3, 0);
    // raras.setString(kImagePath, '');
    _loadData();
  }
  ...
  void _clicked(VoidCallback callback) {
                        ...
                        onPressed: (() {
                          ...
                          if (Provider.of<Status>(context, listen: false)
                              ...
                              Provider.of<IsNotGreen>(context, listen: false)
                                  .isnotgreen) {
                            context.read<Status>().chg('d3', 3,uid);
    ...
    raras.setInt(kbcc3, _beadCounter);
    raras.setInt(kbrc3, _RoundCounter);
