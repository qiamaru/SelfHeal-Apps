...
import 'package:test_firebaseauth/game/Day4/cognitive4/cognitive4.dart';
import 'package:test_firebaseauth/game/Day4/meditate4/meditate4.dart';
...
class _Day4State extends State<Day4> {
  Future methodDay4() async {
    ...
    var determiner = ((pref.getInt('firstday$uid')! + 3) % 7);
    print(pref.containsKey('entered4$uid'));
     if (determiner == 0) {
      determiner = 7;
    }
    // pref.remove('entered4');

    if ( DateTime.now().weekday == determiner) {
      pref.setBool('entered4$uid', true);
      ref.update({"entered4": true});
    }

    if (pref.containsKey('entered4$uid') &&
        (pref.getInt('stateDay4$uid') == 0 ||
            !pref.containsKey('stateDay4$uid'))) {
      context.read<Status>().chg('d4', 1, uid);
    } else if (pref.containsKey('entered4$uid') &&
        (pref.getInt('stateDay4$uid') != 0 &&
            pref.containsKey('stateDay4$uid'))) {
      context.read<Status>().chg('d4', pref.getInt('stateDay4$uid')!, uid);
    } else {
      context.read<Status>().chg('d4', 0, uid);
    }
  }
...
//   Future methodDay4() async {
//     SharedPreferences pref = await SharedPreferences.getInstance();
//     var determiner = ((pref.getInt('UIDfirstday')! + 3) % 7);
//     if (determiner == 0) {
//       if (pref.getInt('currentDay') == 7) {
//         context.read<Status>().chg('d4', 1);
//       }
//     } else {
...
//   }
  ...
  Widget build(BuildContext context) => ListView(
        children: [
          tasktop(
              context,
              context.watch<Status>().stat4,
              //context.read<IsNotGreen>().chg,
              Provider.of<IsNotGreen>(context)),
          taskcenter(context.watch<Status>().stat4),
          taskbottom(context, context.watch<Status>().stat4,
          ...
          children: [
            ElevatedButton(
                onPressed: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const Cognitive4()),
                  );
                },
                style: ElevatedButton.styleFrom(
                    maximumSize: const Size(110, 100),
          ...
          children: [
            ...
            ElevatedButton(
                onPressed: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const Meditate4()),
                  );
                },
                style: ElevatedButton.styleFrom(
                    maximumSize: const Size(110, 100),
