...
import 'package:test_firebaseauth/game/Day7/cognitive7/cognitive7.dart';
import 'package:test_firebaseauth/game/Day7/meditate7/meditate7.dart';
import 'package:shared_preferences/shared_preferences.dart';

import '../reset_dialogue.dart';
...
class _Day7State extends State<Day7> {
  bool complete = false;
  Future methodDay7() async {
    final String uid = FirebaseAuth.instance.currentUser!.uid;

    SharedPreferences pref = await SharedPreferences.getInstance();
    DatabaseReference ref = FirebaseDatabase.instance.ref("users/$uid");

    var determiner = ((pref.getInt('firstday$uid')! + 6) % 7);
    print(pref.containsKey('entered7$uid'));
    if (determiner == 0) {
      determiner = 7;
    }
    // pref.remove('entered7');

    // if (determiner == 0 || DateTime.now().weekday == determiner) {
    //   pref.setBool('entered7$uid', true);
    //   ref.update({"entered7": true});
    // }
    ...
    if (pref.containsKey('entered7$uid') &&
        (pref.getInt('stateDay7$uid') == 0 ||
            !pref.containsKey('stateDay7$uid'))) {
      context.read<Status>().chg('d7', 1, uid);
    } else if (pref.containsKey('entered7$uid') &&
        (pref.getInt('stateDay7$uid') != 0 &&
            pref.containsKey('stateDay7$uid'))) {
      context.read<Status>().chg('d7', pref.getInt('stateDay7$uid')!, uid);
    } else {
      context.read<Status>().chg('d7', 0, uid);
    }
  }

  Future completeMethod() async {
    final String uid = FirebaseAuth.instance.currentUser!.uid;

    SharedPreferences pref = await SharedPreferences.getInstance();
    if (pref.getInt('stateDay7$uid') == 5) {
      setState(() {
        complete = true;
      });
    }
  ...
  Widget build(BuildContext context) => ListView(
        children: [
          tasktop(
              context,
              context.watch<Status>().stat7,
              //context.read<IsNotGreen>().chg,
              Provider.of<IsNotGreen>(context)),
          taskcenter(context.watch<Status>().stat7),
          taskbottom(context, context.watch<Status>().stat7,
              ...
              ElevatedButton(
                  onPressed: () {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => const Cognitive7()),
                    );
                  },
                  style: ElevatedButton.styleFrom(
                      maximumSize: const Size(110, 100),
              ...
              ElevatedButton(
                  onPressed: () {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => const Meditate7()),
                    );
                  },
                  style: ElevatedButton.styleFrom(
                      maximumSize: const Size(110, 100),
